...
def DARK_IMAGES(self,master):
	...
	def dark_image(*event):
		...
		for j in range(len(DARK_images)):
			if j==0:
				DARK_data=np.array(read_image(DARK_images[j]))
			else:
				DARK_data=np.dstack((DARK_data,np.array(read_image(DARK_images[j]))))

		#Cut image
		DARK_x_1,DARK_x_2,DARK_y_1,DARK_y_2=self.Xmin_value.get(),self.Xmax_value.get(),self.Ymin_value.get(),self.Ymax_value.get()
		...
		y, x = np.mgrid[slice(0,len(MASTER_DARK[:,0]), 1) ,slice(0, len(MASTER_DARK[0,:]), 1) ]
	...
	def dark_std(*event):
		...
		y, x = np.mgrid[slice(0,len(MASTER_DARK_std[:,0]), 1) ,slice(0, len(MASTER_DARK_std[0,:]), 1) ]
		cmap = plt.get_cmap('hot')
